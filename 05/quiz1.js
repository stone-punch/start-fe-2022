...
var $total_page_count = document.querySelector('.total_page_count');
...
var list_count = 3;
var page = 1;
var total_page_count = Math.ceil(todayPhotos.length / list_count);
...
function get_start_index(page){
    ...
    // 2page = 3, 5
    // 3page = 6, 8
    var start = (page - 1) * list_count;
    return start;
}

function print_img(page){
    var str = '';
    var start_index = get_start_index(page);
    var end = start_index + list_count;
...
function next_page(){
    var is_end_page = page > total_page_count-1;
    if(is_end_page) return;
    page++;
    print_img(page);
}
...
function init(){
    ...
    $total_page_count.innerHTML = total_page_count;
}
